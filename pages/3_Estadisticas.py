...
st.set_page_config(page_title="Estadisticas — Trading Journal", layout="wide", page_icon="📊")
...
if df.empty:
    st.info("Sin datos todavia. Carga tus primeros trades en el Calendario.")
    st.stop()
...
k8.metric("Racha ganadora max.", f"{m['consecutive_wins']} trades")
...
for i, row in by_side.iterrows():
    ...
    icon = "Long" if row["side"] == "long" else "Short"
    col.metric(f"{icon} P&L", f"${row['pnl']:,.2f}")
    col.metric(f"{icon} Win Rate", f"{row['win_rate']:.1f}%")
    col.metric(f"{icon} Trades", str(row['trades']))
